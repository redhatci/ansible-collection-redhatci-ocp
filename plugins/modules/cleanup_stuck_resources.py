...
description:
  - Discovers all namespaced API resources in a Kubernetes cluster and
    identifies those stuck in termination (have a deletionTimestamp and
...
def find_stuck_resources(namespace, skip_api_resources, module):
    ...
    for group, version, resources in api_groups:
        if isinstance(resources, list) and len(resources) > 0:
            for res in resources:
                ...
                # Must support list, get, and update
                required_verbs = {"list", "get", "update"}
                if not required_verbs.issubset(set(res_verbs)):
                    continue
                ...
                for item in items:
                    if hasattr(item, "metadata"):
                        meta = item.metadata
                        deletion_ts = getattr(meta, "deletionTimestamp",
                                              None)
                        finalizers = getattr(meta, "finalizers", None)
                        item_name = getattr(meta, "name", "")
                        item_ns = getattr(meta, "namespace", namespace)
...
def main():
    ...
    try:
        resources, warnings = find_stuck_resources(
            namespace, skip_api_resources, module
        )
    except Exception as e:
        module.fail_json(
