...
"""Ansible filter plugin that merges list of JUnit converted JSON files into one JSON"""
...
DOCUMENTATION = r"""
  name: reportsmerger
  version_added: "1.0.0"
  short_description: merge multiple test report JSON files into a single test report
  description: >
    This filter plugin merges multiple test report JSON files (created by junit2obj filter)
    into a single consolidated test report. It combines all test suites and recalculates
...
- name: merge reports
  ansible.builtin.set_fact:
    merged: "{{ ['report1.json', 'report2.json', 'report3.json'] | redhatci.ocp.reportsmerger }}"
...
class FilterModule:
    ...
    def reportsmerger(self, filenames: list[str], output_file: None | str = None) -> dict | str:
        """
        ...
        Args:
            ...
            output_file: Optional output file path. If provided, writes merged data to file
                        and returns the file path instead of the data

        Returns:
            dict: Merged test report with aggregated statistics (if no output_file)
        ...
        for filename in filenames:
            if not os.path.exists(filename):
                display.warning(
                    f"reportsmerger plugin: file {filename} does not exist and was skipped"
                )
                continue
    ...
    def _load_and_validate_report(self, filename: str, display) -> dict:
        ...
        if not isinstance(report_data, dict):
            msg = f"Invalid report format in {filename}: expected dict"
            display.error(msg)
    ...
    def _accumulate_report_stats(self, report_data: dict, merged_report: dict, filename: str, display) -> None:
        ...
        for field in ["time", "tests", "failures", "errors", "skipped"]:
            ...
            if value is not None:
                try:
                    ...
                    display.error(f"Invalid numeric data in {filename}: {e}")
                    raise ValueError(f"Invalid numeric data in {filename}: {e}") from e

        # Merge test suites
        test_suites = report_data.get("test_suites", [])
        if not isinstance(test_suites, list):
            display.error(f"Invalid 'test_suites' format in {filename}: expected list")
            raise ValueError(f"Invalid 'test_suites' format in {filename}: expected list")

        merged_report["test_suites"].extend(test_suites)
        display.vv(f"Successfully processed {filename}: {report_data.get('tests', 0)} tests")
