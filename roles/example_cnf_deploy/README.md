...
| ecd_trex_ip_list                      | ["",""]                                                                                              | no       | No default static IP addresses are provided for TRex by default. Must be provided on `ecd_network_config_file` for Grout. If using TestPMD, it is not required |
| ecd_cnfapp_ip_list                    | ["",""]                                                                                              | no       | No default static IP addresses are provided for CNFApp by default. Must be provided on `ecd_network_config_file` for Grout. If using TestPMD, it is not required |
...
To apply a proper network configuration for both CNFApp and TRex, you need to follow a YAML structure like this:
