...
Role tasks:
  ...
  - Initalizes Gitea along with a default user.
...
| sg_username           | *undefined* | if sg_repository is defined | Gitea's initial username             |
| sg_password           | *undefined* | if sg_username is defined   | Gitea's initial password             |
| sg_email              | *undefined* | if sg_uasername is defined  | E-mail address for the initial user  |
| sg_repository         | *undefined* | No       | Initial repository name                                 |
...
## Role requirements
  - The Ansible runner host must have access to the registry containing the *{{ sg_gitea_image }}* (default location is docker.io).
