...
test\_network\_function\_version        | v4.1.4                                               | CNF Cert Suite version downloaded. DCI App Agent supports the latest stable version, which is v4.1.4. HEAD version (in the main branch) can be also used, but it is not guaranteed a complete compatibility with the latest unstable changes. The versions prior to v4.0.0 are not compatible with the agent.
