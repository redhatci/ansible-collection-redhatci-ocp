...
| ------------------------- | ----------------------------------- | -------- | -----------
| ni_oc_tool_path           | `oc`                                | No       | Path to the oc tool binary
...
| ni_job_id                 | `latest`                            | No       | Job ID used for tagging container images in disconnected mode
...
- name: Gather node information in disconnected mode
  ...
  vars:
    ...
    ni_job_id: "{{ ansible_date_time.epoch }}"
```
