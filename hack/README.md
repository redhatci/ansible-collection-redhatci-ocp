...
host:ocp user$ # join_by joins array with "joiner" string
host:ocp user$ join_by '+' "${x[@]}"
a+b+c d+e
host:ocp user$ # for example we can print out a list:
host:ocp user$ echo "list: ['$(join_by "', '" "${x[@]}")']"
list: ['a', 'b', 'c d', 'e']
host:ocp user$ # contains_element <item> <items> (checks whether item is in items)
...
host:ocp user$ utils.run_cmd 0 true # 1st argument is expected good return code
host:ocp user$ utils.run_cmd 1 true # will exit
logout
host:ocp user$ # if this is set: DO_NOT_EXIT=1 log.die() returns instead of exit, causing utils.run_cmd() to stay in the shell
host:ocp user$ # DO_NOT_EXIT=1 allows failing run_cmd to return instead of exit
host:ocp user$ # don't forget to unset it:
host:ocp user$ source hack/base.bash
host:ocp user$ utils.run_cmd 0 true
host:ocp user$ echo $?
0
host:ocp user$ DO_NOT_EXIT=1
host:ocp user$ utils.run_cmd 0 false
Thu Jul 24 15:49:40 UTC 2025 - FATAL - The command 'false' returned unexpected rc=1. [EXPECTED: 0]
host:ocp user$ echo $?
1
host:ocp user$ DO_NOT_EXIT=0
host:ocp user$ utils.run_cmd 0 false
Thu Jul 24 15:49:40 UTC 2025 - FATAL - The command 'false' returned unexpected rc=1. [EXPECTED: 0]
logout
```
