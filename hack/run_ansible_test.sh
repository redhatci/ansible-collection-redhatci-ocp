...
run_tests() {
  ...
  for test_type in "${TEST_TYPES[@]}"; do
    case "${test_type}" in
      sanity)
        ansible-test "${test_type}" "${EXCLUDE[@]}" --verbose --docker --python "${version}" --color --coverage --failure-ok --lint
        ;;
      units)
        ansible-test "${test_type}" --verbose --docker --python "${version}" --color --coverage || :
        ;;
      integration)
        ansible-test "${test_type}" --verbose --docker --python "${version}" --color --coverage || :
        ;;
...
for xversion in "${PY_EXCLUDE_VERSIONS[@]}"; do
  RESULT=$(echo "${RESULT}" | "${SED}" -e "s/${xversion} //g")
done
IFS=$' ' read -r -a PY_VERS <<< "${RESULT}"
