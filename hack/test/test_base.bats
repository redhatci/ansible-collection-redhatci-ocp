...
# --- Unit Tests for utils.run_cmd ---

@test "test utils.run_cmd with expected_rc=0" {
  expected="Hello, world."
  run bash -c "source hack/base.bash && utils.run_cmd 0 echo '${expected}'"
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "${expected}" ]]
}

@test "test utils.run_cmd with expected_rc=1" {
  run bash -c "source hack/base.bash && utils.run_cmd 1 false"
  [[ "${status}" -eq 1 ]]
  [[ "${output}" == "" ]]
}

@test "test utils.run_cmd with expected_rc=1 and DO_NOT_EXIT=1" {
  run --separate-stderr bash -c "source hack/base.bash && export DO_NOT_EXIT=1 && utils.run_cmd 0 false"
  [[ "${status}" -eq 1 ]]
  [[ "${output}" == "" ]]
...
@test "test utils.run_cmd with DO_NOT_EXIT=0 (default) - function calls exit" {
  # When DO_NOT_EXIT=0 (default), log.die should call exit() and terminate the process
  run --separate-stderr bash -c "source hack/base.bash && unset DO_NOT_EXIT && utils.run_cmd 0 false"
...
@test "test utils.run_cmd with DO_NOT_EXIT=1 - function returns without exit" {
...
@test "test utils.tools_setup('Linux') assigning FIND=find, GREP=sed, SED=sed" {
  OS="Linux"
  FIND="find"
  GREP="grep"
  SED="sed"
...
@test "join_by works with empty array" {
  run bash -c 'source hack/base.bash && join_by ","'
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "" ]]
}

@test "join_by works with single element array" {
  run bash -c 'source hack/base.bash && join_by "," "a"'
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "a" ]]
}

@test "join_by works with multiple elements" {
  run bash -c 'source hack/base.bash && join_by "," "a" "b" "c"'
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "a,b,c" ]]
}

@test "join_by works with empty separator" {
  run bash -c 'source hack/base.bash && join_by "" "a" "b" "c"'
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "abc" ]]
}

@test "join_by works with formatting separator" {
  run bash -c 'source hack/base.bash && join_by "%,%" "a" "b" "c"'
  [[ "${status}" -eq 0 ]]
  [[ "${output}" == "a%,%b%,%c" ]]
}
